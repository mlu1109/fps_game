...
class Renderer
{
  ...
  public:
    ...
    // Drawing
    void pre();
    void renderCubemap(const std::string &shader, const std::string &cubemap, const std::string &vertexArray, const glm::mat4 &modelWorld);
    void renderCubemap(const Drawable &drawable) { renderCubemap(drawable.getShader(), drawable.getTexture(), drawable.getVertexArray(), drawable.getModelWorld()); }
